...
from tkcalendar import Calendar, DateEntry #for order and rundate calendar
...
def operator_placeholder_delete(event):
    ...
    return None
def user_placeholder_delete(event):
    user_input.configure(state=NORMAL)
    user_input.delete(0, END)  # delete all user input placeholder content
    return None
...
#user codes
user_label = Label(root, text="USER:", font = fontstyle, bg = "#ffffff", fg = "#072b60", anchor= "e", width = "15") #user label
user_label.grid(row = 1, column = 0) #user label grid
user_input = Entry(root, width = 20) #user input
user_input.grid(row = 1, column = 1) #user input grid
user_input.insert(0, "ex: Min Gu Cho") #user input placeholder
user_input.bind("<Button-1>",user_placeholder_delete) #when button clicked delete user input placeholder
...
orderID_input.insert(0, "ex: N0001") #orderID input placeholder
...
order_volume_input.insert(0, "ex: 8 (max: 96)") #order volume input placeholder
...
rundate_label = Label(root, text = "RUN DATE:", font = fontstyle, bg = "#ffffff", fg = "#072b60", anchor= "e", width = "15") #rundate label
rundate_label.grid(row = 9, column = 0) #rundate label grid
rundate_calendar = DateEntry(root,) #rundate calendar
rundate_calendar.grid(row = 9, column = 1) #rundate calendar grid
...
def onClick():
    ...
    rundate = rundate_calendar.get()
    rundate = rundate.replace(". ","-")
    rundate = rundate.replace(".","")
    file_name = file_name_input.get()
    #send gathered data to bridge.py
    bridge.start(current_directory, operator, user, orderID, order_volume, well_new_or_old, wellID, well_position, step, order_date, rundate, file_name)
