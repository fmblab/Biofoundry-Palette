...
def main(name, order_number, position, parent_path):
    #calculation
    column = float(order_number)/8
    column_whole = int(column)
    column_des = float(str(column-int(column))[1:])
    column_des = int(column_des * 8)
    position = int(position)
    repeating_wells(name, position, column_whole, column_des, parent_path)
